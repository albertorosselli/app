...
// ---------- Core audit ----------
export async function runAudit(input, adapters = defaultAdapters) {
...
function buildSuggestions(gbp, site, localPack) {
    const s = [];
    if (!localPack) {
    ...
    if (!isUnavailable(gbp)) {
        if (!gbp.isVerified)
            s.push('Press verify your Google Business Profile.');
        if (!gbp.primaryCategory)
            s.push('Update primary category to match your service.');
        if (!gbp.descriptionExists)
            s.push('Add a clear business description in GBP.');
        if (!gbp.photos)
            s.push('Upload quality photos to GBP.');
        if (!gbp.websiteLinked)
            s.push('Link to website from GBP.');
        if (!gbp.phonePresent)
            s.push('Add phone number to GBP.');
        if (!gbp.hoursValid)
            s.push('Add or correct opening hours in GBP.');
    }
    else {
        s.push('Complete a manual Google Business Profile check (adapter not configured).');
    }
    if (!isUnavailable(site)) {
        if (!site.heroServiceStatement)
            s.push('Make your homepage communicate service in the first 5 seconds.');
        if (!site.heroPhoneVisible)
            s.push('Show a tap-to-call number above the fold on mobile.');
        if (site.loadsFastMs !== null && site.loadsFastMs > 2500)
            s.push('Improve mobile load speed; aim under 2.5s.');
    }
    else {
